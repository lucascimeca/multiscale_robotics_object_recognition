...
class WRSDataProvider(OneOfKDataProvider):
    """Data provider for WRS object images."""
    path_to_data = 'data/np_data/'
    ...
    def __init__(self, which_set='train', batch_size=100, max_num_batches=-1,
        ...
        data_path = os.path.join(self.path_to_data, '{}_set/data.npz'.format(which_set))
        assert os.path.isfile(data_path), (
            'Data file does not exist at expected path: ' + data_path
        )
        # load data from compressed numpy file
        loaded = np.load(data_path)
        if loaded['format'] == 'gray':
            self.channel_dimension = 1
        else:
            self.channel_dimension = 3
        ...
        if input_num:
            inputs = inputs[:input_num]
            targets = targets[:input_num]
        inputs = inputs if convolution == False else inputs.reshape(inputs.shape[0], 300, 300, self.channel_dimension)
        targets = targets.astype(np.int32)
        # pass the loaded data to the parent class __init__
        super(WRSDataProvider, self).__init__(
            inputs, targets, batch_size, max_num_batches, shuffle_order, rng)



class YOLODataProvider(OneOfKDataProvider):
    """Data provider for WRS object images."""
    path_to_data = 'data/np_data/'
    ...
    def __init__(self, which_set='train', batch_size=100, max_num_batches=-1,
        ...
        # MLP_DATA_DIR environment variable should point to the data directory
        self.data_set_idx = 0
        self.datasets = [str(file).split('_')[-1].split('.')[0]
                         for file in pathlib.Path(self.path_to_data + which_set + "_set").glob('**/*.npz')]

        data_path = os.path.join(self.path_to_data, '{}_set/data_{}.npz'.format(
            which_set,
            self.datasets[self.data_set_idx])
                                 )
        assert os.path.isfile(data_path), (
            'Data file does not exist at expected path: ' + data_path
        )
        ...
        if loaded['format'] == 'gray':
            ...
            self.channel_dimension = 3
        self.num_classes = loaded['num_classes']
        inputs, targets = loaded['inputs'][:loaded['idx_limit'], :], loaded['targets'][:loaded['idx_limit']]
        # label map gives strings corresponding to integer label targets
        self.label_map = loaded['label_map']
        if input_num:
            inputs = inputs[:input_num]
            targets = targets[:input_num]
        inputs = inputs if convolution == False else inputs.reshape(inputs.shape[0], 640, 640, self.channel_dimension)
        targets = targets.astype(np.int32)
        # pass the loaded data to the parent class __init__
        super(YOLODataProvider, self).__init__(
            inputs, targets, batch_size, max_num_batches, shuffle_order, rng)
    ...
    def next(self):
        ...
        if self._curr_batch + 1 > self.num_batches:
            # no more batches in current iteration through data set so start
            # new epoch ready for another pass and indicate iteration is at end
            if self.data_set_idx >= len(self.datasets)-1:
                self.data_set_idx = 0
                print("DATA PROVIDER: restarting data_sets")
            else:
                self.data_set_idx += 1
                print("DATA PROVIDER: getting next dataset")
            data_path = os.path.join(
                self.path_to_data,
                '{}_set/data_{}.npz'.format(self.which_set, self.datasets[self.data_set_idx])
            )
            ...
            # label map gives strings corresponding to integer label targets
            self.label_map = loaded['label_map']
            inputs = inputs if self.convolution == False else inputs.reshape(inputs.shape[0], 640, 640,
                                                                        self.channel_dimension)
            targets = targets.astype(np.int32)
            # pass the loaded data to the parent class __init__
            self.inputs = inputs
            self.targets = targets
            self._max_num_batches = -1
            self._update_num_batches()
            self.shuffle_order = True
            self._current_order = np.arange(inputs.shape[0])
            self.new_epoch()
            raise StopIteration()
        # create an index slice corresponding to current batch number
